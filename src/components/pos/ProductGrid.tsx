...
export const ProductGrid: React.FC<ProductGridProps> = ({ productos, loading }) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden p-4 sm:p-6 gap-4 sm:gap-6">
      {/* Filtros superiores */}
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center bg-white border border-slate-200 rounded-lg p-4 flex-shrink-0">
        
        {/* Categorías con iconos - Scroll horizontal en móviles */}
        <div className="flex overflow-x-auto gap-2 pb-2 lg:pb-0 w-full lg:flex-wrap lg:overflow-visible scrollbar-none">
          {categorias.map((cat) => {
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 px-3 py-1.5 sm:px-4 sm:py-2 rounded-md text-xs sm:text-sm font-semibold transition-all duration-150 cursor-pointer flex items-center gap-2 border select-none ${
                  isSelected
                    ? 'bg-slate-700 border-slate-700 text-white shadow-xs'
                    : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100 hover:text-slate-800'
                }`}
              >
                <Icon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${isSelected ? 'text-white' : 'text-slate-500'}`} />
                {cat}
              </button>
            );
          })}
        ...
        <div className="relative w-full lg:w-72 flex-shrink-0">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-1.5 sm:py-2 bg-slate-50 border border-slate-200 rounded-md text-xs sm:text-sm focus:outline-none focus:bg-white focus:border-slate-500"
          />
        </div>
      </div>
            ...
            {filteredProductos.map((p) => {
              ...
              return (
                <button
                  ...
                  disabled={isOutStock}
                  onClick={() => addToCart(p)}
                  className={`flex flex-col text-left bg-white border rounded-lg p-3 sm:p-4 transition-all duration-150 select-none group relative ${
                    isOutStock
                ...
                >
                  {/* Categoría Tag */}
                  <span className="text-3xs sm:text-2xs font-bold uppercase tracking-wider text-slate-400 mb-1">
                    {p.categoria}
                  </span>

                  {/* Nombre */}
                  <h3 className="font-semibold text-slate-800 text-sm sm:text-base leading-tight mb-2 group-hover:text-slate-900">
                    {p.nombre}
                  </h3>
                  ...
                  <div className="mt-auto flex justify-between items-end w-full pt-2">
                    ...
                    {/* Stock status */}
                    <span
                      className={`text-3xs sm:text-xs font-semibold px-2 py-0.5 rounded-full ${
                        isOutStock
